...
from telegram.ext import Updater, CommandHandler, MessageHandler, Filters, ThreadedDispatcher
...
OPENAI_API_KEY = "sk-I2KnT3BlbkFJyEwFBqqlQ5MzlDshc8cB"
openai.api_key = OPENAI_API_KEY

SECRET_KEY = "live_MRRA6IBEnkvh8IG2OQ8fNPhZ-Q"
Configuration.account_id = "066"
...
cred = credentials.Certificate("telegabot-16d594244d.json")
initialize_app(cred, {'databaseURL': 'https://telegabst1.firebasedatabase.app/'})
...
def send_message_and_get_response_to_user_question(update: Update, message):
    ...
    try:
        ...
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",  # Выберите движок, который вы хотите использовать
            messages=[
                {"role": "system", "content": "Ты являешься gpt-3.5-turbo!"},
                {"role": "user", "content": prompt},
            ],
            max_tokens=4000,  # Установите максимальное количество токенов в ответе
            n=1,
            temperature=0,
        )
        usage = response["usage"]
        tokens_used = usage["total_tokens"]
        answer = response['choices'][0]['message']['content'] + tokens_used
    except Exception as e:
        bot = update.effective_message.bot
        error_message = f"Ошибка: {str(e)}\n\n{traceback.format_exc()}"
        bot.send_message(chat_id=5718940340, text=error_message)
        return "Извиняюсь, у меня возникла какая-то ошибка! Прошу вас повторить запрос позднее..."
    return answer
...
def main():
    ...
    dp.add_handler(MessageHandler(Filters.regex('^Очистить переписку$'), handle_menu))
